...
Query Query::Filter(FieldPath field_path,
  ...
  if (field_path.IsKeyFieldPath()) {
    if (IsArrayOperator(op)) {
      ...
      auto* element = value.array_value.values;
      auto* end = value.array_value.values + value.array_value.values_count;
      for (; element != end; ++element) {
        *element = ParseExpectedReferenceValue(*element, type_describer);
      }
    } else {
...
void Query::ValidateDisjunctiveFilterElements(
  ...
  HARD_ASSERT(
      GetTypeOrder(field_value) == TypeOrder::kArray,
      "A FieldValue of Array type is required for disjunctive filters.");
  if (field_value.array_value.values_count == 0) {
    ThrowInvalidArgument(
        "Invalid Query. A non-empty array is required for '%s'"
        " filters.",
        Describe(op));
  }
  if (field_value.array_value.values_count > 10) {
    ThrowInvalidArgument(
